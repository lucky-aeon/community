...
func (*UserTag) AssignUserLabel(userId int, tags []int) (tagNames []string) {
	...
	if len(tags) != 0 {
		...
		model.UserTag().Where("id in ?", tags).Select("name").Find(&tagNames)
	}
	return
}

func (*UserTag) GetTagsByUserId(userId int) (tagNames []string) {
	var tagsIds []int
	model.UserTagRelation().Where("user_id = ?", userId).Select("user_tag_id").Find(&tagsIds)
	if len(tagsIds) == 0 {
		return
	}
	model.UserTag().Where("id in ?", tagsIds).Select("name").Find(&tagNames)
	return tagNames
}
